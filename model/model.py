...
def createModel(pretrained=True, preChannel=3, channel=3, preNumClasses=1000, numClasses=2, weightPath=None, device=None):
    ...
    if pretrained:
        model.patch_embed.proj = nn.Conv2d(in_channels= preChannel, out_channels=128, kernel_size=(4, 4), stride=(4, 4)).cuda()
        model.head = nn.Linear(in_features=1024, out_features=preNumClasses, bias=True).cuda()
        
        ## <originWeightpath로 처음 학습시킬 때 사용하는 로드
        # ckpt_path = os.path.join(weightPath, "weight.pth")
        ...
        model.patch_embed.proj = nn.Conv2d(in_channels=channel, out_channels=128, kernel_size=(4, 4), stride=(4, 4)).cuda()
        model.head = nn.Linear(in_features=1024, out_features=numClasses, bias=True).cuda()
